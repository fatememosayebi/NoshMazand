...
const Radio = ({ title, items, value, onChange, w = "12", dir = "rtl", textAlign = "right", afterText, disabled = false, mask, placeholder = "" }: RadioProps)=> {
    return (
            ...
            {items.map((x, i) => {
                return (<label className="flex items-center cursor-pointer" key={i}>
                    <input type="radio" className="form-radio"
                        onChange={(e) => {
                            console.log(e)
                            if (onChange) onChange(x.id, x)
                        }}
                        checked={value== x.id }
                        disabled={disabled} />
                    <span className="text-base font-light pt-1 ">{x.title}</span>
                </label>)
            })}
        </div>
        {!!afterText &&
